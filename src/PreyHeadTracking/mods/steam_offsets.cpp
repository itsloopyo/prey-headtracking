// Every Steam build of Prey this mod knows about. Append-only: a profile is
// added when a patch lands and never edited, so a player who has not taken the
// patch keeps matching the profile they already work on.
