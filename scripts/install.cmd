...
:: Written only when it is not already there, so re-running the installer to
:: update the mod keeps whatever the user tuned. Listing it in MOD_DLLS put it
:: through the unconditional copy /y below and reset every key on every update.
:: launcher-manifest.json delivers the same file the same way, through its
:: write-if-absent loader.seed block - the two paths have to agree.
...
for %%v in (GAME_ID MOD_DISPLAY_NAME MOD_INTERNAL_NAME STATE_FILE FRAMEWORK_TYPE MOD_DLLS MOD_SEED_FILES ASI_LOADER_NAME) do (
    if not defined %%v (
